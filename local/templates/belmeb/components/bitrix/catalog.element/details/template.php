...
?>
    ...
    <section class='product'>
        <div class="product-card">
            ...
            <div class="product-slider__pager">
                <span class="product-slider__next"></span>
                <div class="product-slider__pager-items">
                    <?
                    if (!empty($actualItem['MORE_PHOTO']))
                    {
                        foreach ($actualItem['MORE_PHOTO'] as $key => $photo)
                        {
                            if ($key === 0){
                                ?>
                                <div>
                                    <a class="product-slider__pager-item">
                            ...
                            ?>
                            <div >
                                <a href="javascript:void(0)" class="product-slider__pager-item"><img src="<?=$photo['SRC']?>" alt=""></a>
                            </div>
                            <?}
                        }
                    }?>
                </div>
                <span class="product-slider__prev"></span>
            </div>
            <div class="product-info">
                <div class="product-info__wrap">
                    <div class="resp-block">
                        <h1><?=$name?></h1>
                        <span class="product-info__vendor">Код товара: <?=$arResult['DISPLAY_PROPERTIES']['CODE']['VALUE']?></span>
                        <div class="product-info__price-wrap">
                            <div class="product-info__price">
                                <span class="product-info__price--main"><?=$price['PRINT_RATIO_PRICE']?></span>
								<?if (!empty($arResult['PROPERTIES']['OLD_PRICE'])) :?>
                                <span class="product-info__price--old"><?=$arResult['PROPERTIES']['OLD_PRICE']['VALUE']?> руб.</span>
								<?endif?>
                            </div>
                            <div class="product-info__buttons" id="<?=$itemIds['BASKET_ACTIONS_ID']?>">
                                <a class="btn btn btn-red" id="<?=$itemIds['BUY_LINK']?>" href="javascript:void(0);" OnClick="window.location='<?=$arResult["ADD_URL"]?>#buy';$.fancybox.open('Товар был добавлен в вашу корзину');">
                                   Купить
                                </a>
                                <a href="javascript:void(0);" class="product-info__fav-btn">в избранное</a>
                            </div>
                        </div>
                    </div>
                    <div class="resp-block">
                        <div class="product-info_tags">
                            <?if(!empty($arResult['PROPERTIES']['COLLECTIONS']['VALUE'])):?>
                                <div class="product-info_tags-item">
                                    <span class="product-info__tags-title">Коллекция:</span>
                                    <?=$arResult['PROPERTIES']['COLLECTIONS']['VALUE']?>
                                </div>
                            <?endif;?>
                            <?if(!empty($arResult['PROPERTIES']['PROIZ']['VALUE'])):?>
                                <div class="product-info_tags-item">
                                    <span class="product-info__tags-title">Производитель:</span>
                                    <?=$arResult['PROPERTIES']['PROIZ']['VALUE']?>
                                </div>
                            <?endif;?>
                        </div>
                        <div class="product-info__composition">
                            <?if(!empty($arResult['PROPERTIES']['SOSTAV']['VALUE'])):?>
                            <span class="product-info__tags-title">Состав гарнитура:</span>
                            <div class="product-info__blocks">
                                <?$co = 0;?>
                                <?foreach ($arResult['PROPERTIES']['SOSTAV']['VALUE'] as $count) :?>

                                    <a class="product-info__block" href="<?=$arResult['PROPERTIES']['SOSTAV']['DESCRIPTION'][$co]?>"><?=$count?></a>
                                    <?$co++?>
                                <?endforeach;?>
                            </div>
                            <?endif;?>
    ...
    <section class="tabs">
        <div class="property-tabs">
            <div class="tabs-panel">
                <a class="" href="javascript:void(0)">характеристики</a><a href="javascript:void(0)" class="active">отзывы</a>
            </div>
                ...
                <div class="tab-content">
                    <ul class="tab-list">
                        <?foreach($arResult['PROPERTIES'] as $prop):
                            if(empty($prop['VALUE']) || !in_array($prop['CODE'], $arParams['PROPERTY_CODE']))
                                continue;
                            ?>
                            <li>
                                <span class="tab-list__item--left"><?=$prop['NAME']?></span>
                                <span class="tab-list__dots"></span>
                                <span class="tab-list__item--right"><?=$prop['VALUE']?></span>
                            </li>
                        <?endforeach;?>
                    </ul>
                    <div class="tab-text">
            ...
            <div class="tab open-tab" style="display: block;">
                <h1>Отзывы</h1>
                <div class="tab-content">
                    <ul class="tab-list">
                        <?foreach($arResult['PROPERTIES'] as $prop):
                    ...
                    <div class="tab-text"><?=$arResult['DETAIL_TEXT'];?>
                    </div>
                </div>
            </div>
        </div>
